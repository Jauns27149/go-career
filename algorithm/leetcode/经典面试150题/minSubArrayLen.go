...
/*
 1. 计算nums数组总和total
 2. 默认数组长度为最小长度，每次循环最小长度result减1
    a. 循环计算当前result长度组合是否满足要求，满足result--, 否返回最小长度result+1
*/
...
start:
	for result, n := len(nums)-1, len(nums)-1; result > 0; result-- {
		total -= nums[n]
		n--
		if total >= target {
			if result == 1 {
		...
		for i, t := 1, total; i+result-1 < len(nums); i++ {
			t = t - nums[i-1] + nums[i+result-1]
			if t >= target {
				if result == 1 {
					return result
				}

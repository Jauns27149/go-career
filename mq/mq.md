...
<column>
	must be one of arguments, auto_delete, consumer_capacity, consumer_utilisation, consumers, disk_reads, disk_writes, durable, exclusive, exclusive_consumer_pid, exclusive_consumer_tag, head_message_timestamp, memory, message_bytes, message_bytes_persistent, message_bytes_ram, message_bytes_ready, message_bytes_unacknowledged, messages, messages_persistent, messages_ram, messages_ready, messages_ready_ram, messages_unacknowledged, messages_unacknowledged_ram, name, owner_pid, pid, policy, slave_pids, state, synchronised_slave_pids, type

--online
	lists only queues on online (reachable) nodes

--offline
	lists only queues on offline (unreachable) nodes

--local
	only return queues hosted on the target node
